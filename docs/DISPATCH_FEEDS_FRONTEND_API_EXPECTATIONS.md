...
| --------- | ------ | -------- | ------------------------ | ------------------------------------------------------------------------- |
| `sort`    | string | No       | Sort field and direction | `created_at`, `-created_at`, `updated_at`, `-updated_at`, `name`, `-name` |
|           |        |          | Default if not specified | `-created_at` (newest first)                                              |
...
```
GET /api/dispatch/feeds?tier=core&enabled=true&sort=-created_at&search=trucking&page=1&limit=25
```
...
3. **Sort default** - If no sort param, use `-created_at`
