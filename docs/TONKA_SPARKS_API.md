# Tonka Sparks API Reference

Blog idea seeds that become TonkaSparkPost entries when processed by n8n WritersRoom workflow.
...
**GET /api/tonka-spark** (auth: bearer or internal)
...
- **TonkaSparks** (this API) - Seed blog ideas stored in `tonka_sparks` collection
- **TonkaSparkPosts** - Full blog posts generated by n8n, stored in `tonka_spark_posts` collection
