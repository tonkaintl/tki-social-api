...
export class YouTubeClient {
  ...
  async makeRequest(method, endpoint, data = null, retryCount = 0) {
    ...
    const options = {
      headers: {
        Authorization: `Bearer ${this.config.YOUTUBE_ACCESS_TOKEN}`,
    ...
    try {
      ...
      if (!response.ok) {
        throw new ApiError(
          ...
          responseData.error?.message || 'YouTube API request failed',
          response.status,
          responseData.error
        );
      }
      ...
      if (this.shouldRetry(error, retryCount)) {
        ...
        logger.info(`Retrying YouTube API request in ${delay}ms`, {
          method,
