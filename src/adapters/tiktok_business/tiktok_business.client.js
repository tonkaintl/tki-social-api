...
export class TikTokBusinessClient {
  ...
  async makeRequest(method, endpoint, data = null, retryCount = 0) {
    ...
    const options = {
      headers: {
        Authorization: `Bearer ${this.config.TIKTOK_BUSINESS_ACCESS_TOKEN}`,
    ...
    try {
      logger.debug('Making TikTok Business API request', {
      ...
      if (!response.ok) {
        throw new ApiError(
          ...
          responseData.error?.message || 'TikTok Business API request failed',
          response.status,
          responseData.error
        );
      }
      ...
      if (this.shouldRetry(error, retryCount)) {
        ...
        logger.info(`Retrying TikTok Business API request in ${delay}ms`, {
          method,
